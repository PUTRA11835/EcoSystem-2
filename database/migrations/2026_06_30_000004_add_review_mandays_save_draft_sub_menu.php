...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $existing = DB::table('menu')->where('slug', 'ticket.review-mandays.save-draft')->first();
        if ($existing) {
            $menuId = $existing->id;
        } else {
            $menuId = DB::table('menu')->insertGetId([
                'parent_id'  => $parent->id,
                'name'       => 'Save Draft',
                'slug'       => 'ticket.review-mandays.save-draft',
                'type'       => 'function',
                'route_name' => null,
                'icon'       => null,
                'order_seq'  => 6,
    ...
    {
        $menu = DB::table('menu')->where('slug', 'ticket.review-mandays.save-draft')->first();
        if ($menu) {
