...
use App\Models\ProjectUpdate; // Model ini kita gunakan untuk isu
...
class IssueController extends Controller
{
    ...
    public function index()
    {
        ...
        $issues = ProjectUpdate::whereIn('id', function($query) {
                $query->selectRaw('max(id)')
                    ->from('project_updates')
                    ->groupBy('project_id');
            })
            ->with(['project.client.basicData']) // Eager load relasi untuk efisiensi
