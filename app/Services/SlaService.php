...
class SlaService
{
        ...
        try {
            ...
            $ticket->sla()->create([
                ...
                'resolution_due_at'         => $slaMode === 'full'
                    ? $validated->copy()->addHours((float) $policy->resolution_hours)
                    : null,
    ...
    public function recordMessageEvent(
        ...
        try {
            ...
            if ($senderType === 'customer') {
                ...
                $this->handleAgentReply($sla, $ticket, $message, $eventAt);
            }
    ...
    // ─── AGENT REPLY ──────────────────────────────────────────────────────────
    // Setiap reply agent → bola ke customer, waiting clock mulai dari reply terakhir.
    // Burst (agent kirim beberapa pesan berturut) → patokan dari pesan terakhir.

    private function handleAgentReply(TicketSla $sla, Ticket $ticket, TicketMessage $message, Carbon $eventAt): void
    {
        $is24h        = (bool) ($sla->policy?->is_24_hours ?? true);
        ...
        // Selalu update sla_paused_at ke pesan agent terbaru (burst: patokan dari pesan terakhir)
        $sla->update([
    ...
    public function closeTicketSla(TicketSla $sla, Ticket $ticket, ?TicketMessage $message, Carbon $closedAt, ?float $lastSessionHours = null): void
    {
        try {
            ...
            $sla->refresh();
            $resolutionStart = $sla->first_responded_at
                ? Carbon::parse($sla->first_responded_at)
                : Carbon::parse($sla->sla_start_at);
        ...
        foreach ($messages->groupBy('ticket_id') as $ticketId => $msgs) {
            ...
            foreach ($msgs as $msg) {
                ...
                if ($msg->sender_type === 'customer') {
                    ...
                    // Update ball holder: setiap agent reply → bola ke customer
                    $ballHolder = 'customer';
                    $pausedAt   = $eventAt;
                }
            }
