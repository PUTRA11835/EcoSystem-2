                        ...
                        <div id="verticalPhasesList" class="space-y-2">
                            @foreach($support->phases()->where('orientation', 'vertical')->orderBy('order_sequence')->get() as $phase)
                                <div class="phase-item bg-white p-3 rounded border border-gray-200"
                                    ...
                                    data-original-weight="{{ $phase->weight ?? 0 }}"
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                        ...
                                        <div class="flex items-center space-x-2 flex-shrink-0">
                                            ...
                                            <button onclick="togglePhaseVisibilityUI({{ $phase->id }})"
                                                ...
                                                title="{{ ($phase->is_visible ?? true) ? 'Visible' : 'Hidden' }}">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path class="eye-open {{ ($phase->is_visible ?? true) ? '' : 'hidden' }}" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                    <path class="eye-closed {{ ($phase->is_visible ?? true) ? 'hidden' : '' }}" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"></path>
                                                </svg>
                                            </button>
...
<script>
// ============================================================================
// PHASE CONFIGURATION - BATCH UPDATE SYSTEM
// ============================================================================
...
/**
* ✅ Calculate and display total weights (exclude deleted items)
*/
...
/**
* ✅ Remove phase (mark for deletion)
*/
...
window.saveAllPhaseChanges = async function() {
    ...
    try {
        ...
        for (const phaseId of window.phaseChanges.toDelete) {
            ...
            await axios.post(`/delivery/support/${supportId}/phases/${phaseId}/delete`, {}, {
        ...
        for (const newPhase of window.phaseChanges.toCreate) {
            ...
            if (createResponse.data.success) {
                const phaseId = createResponse.data.phase.id;
        ...
        for (const phaseId of window.phaseChanges.modified) {
            ...
            if (Object.keys(updates).length > 0) {
                await axios.put(`/delivery/support/${supportId}/phases/${phaseId}/update`, {
                    ...updates,
                    _token: token
                });
            }
            ...
            if (window.phaseChanges.visibilityChanges[phaseId] !== undefined) {
                await axios.post(`/delivery/support/${supportId}/phases/${phaseId}/toggle-visibility`, {
                    _token: token
                });
            }
        }
