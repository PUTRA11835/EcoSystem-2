                        ...
                        <div id="verticalPhasesList" class="space-y-2">
                            @foreach($verticalPhases as $phase)
                                <div class="phase-item bg-white p-3 rounded border border-gray-200" 
                                     ...
                                     data-original-weight="{{ $phase->pivot->weight }}"
                                     data-original-visible="{{ $phase->pivot->is_visible ? '1' : '0' }}"
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                        <div class="flex items-center space-x-3 flex-1 min-w-0">
                                            ...
                                            @if($phase->pivot->is_golive_phase)
                                        ...
                                        <div class="flex items-center space-x-2 flex-shrink-0">
                                            ...
                                            <button onclick="toggleGoLivePhase({{ $phase->id }})" 
                                                    data-phase-id="{{ $phase->id }}"
                                                    class="golive-btn p-1 {{ $phase->pivot->is_golive_phase ? 'text-green-600' : 'text-gray-400' }}"
                                                    title="{{ $phase->pivot->is_golive_phase ? 'Go-Live Phase' : 'Mark as Go-Live Phase' }}">
                                            ...
                                            <button onclick="togglePhaseVisibilityUI({{ $phase->id }})" 
                                                    data-phase-id="{{ $phase->id }}"
                                                    class="visibility-btn p-1 {{ $phase->pivot->is_visible ? 'text-blue-600' : 'text-gray-400' }}"
                                                    title="{{ $phase->pivot->is_visible ? 'Visible' : 'Hidden' }}">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path class="eye-open {{ $phase->pivot->is_visible ? '' : 'hidden' }}" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                                    <path class="eye-closed {{ $phase->pivot->is_visible ? 'hidden' : '' }}" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"></path>
                                                </svg>
                                            </button>
