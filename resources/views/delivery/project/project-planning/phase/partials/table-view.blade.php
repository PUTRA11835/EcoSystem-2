<!-- ✅ FULL RESPONSIVE TABLE VIEW - All Original Code Preserved -->
...
(function() {
    ...
    function getProjectId() {
        ...
        console.error('❌ Cannot determine project ID from any source!');
        return null;
    }
    
    // ✅ EXPOSE GLOBALLY SO IT CAN BE CALLED FROM MAIN SCRIPT
    window.loadAllPhasesData = function() {
        const projectId = getProjectId();
        if (!projectId) {
            console.error('❌ Cannot load data: Project ID not found');
            return;
        }
    ...
    function saveExpandedState() {
        ...
        document.querySelectorAll('.phase-toggle').forEach(function(button) {
            const icon = button.querySelector('svg');
            if (icon && icon.classList.contains('rotate-90')) {
                const onclick = button.getAttribute('onclick');
                const idMatch = onclick.match(/togglePhase\((\d+)/);
                if (idMatch) state.phases.push(parseInt(idMatch[1]));
            }
        });
        ...
        document.querySelectorAll('.group-toggle').forEach(function(button) {
            ...
            if (icon && icon.classList.contains('rotate-90')) {
                const onclick = button.getAttribute('onclick');
                const idMatch = onclick.match(/toggleGroup\((\d+)/);
                if (idMatch) state.groups.push(parseInt(idMatch[1]));
            }
        });

        document.querySelectorAll('.stage-toggle').forEach(function(button) {
            const icon = button.querySelector('svg');
            if (icon && icon.classList.contains('rotate-90')) {
                const onclick = button.getAttribute('onclick');
                const idMatch = onclick.match(/toggleStage\((\d+)/);
                if (idMatch) state.stages.push(parseInt(idMatch[1]));
            }
        });
                ...
                <tr>
                    <td colspan="11" class="text-center py-8 sm:py-12">
                        <div class="text-gray-500">
                            ...
                            <p class="mt-1 text-xs text-gray-500">Add phases in project configuration.</p>
                        </div>
                    </td>
                </tr>
            `;
...
@media (max-width: 1024px) {
    ...
    /* Better sticky column on mobile */
    th.sticky, td.sticky {
