    ...
    <div>
        ...
        <div class="grid grid-cols-6 gap-4 form-grid">
            ...
            <div class="col-span-1">
                ...
                <input type="text" id="identificationNumber" required class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Responsible Institution -->
            <div class="col-span-1">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Responsible Institution</label>
                <input type="text" id="responsibleInstitution" placeholder="e.g., Disdukcapil" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Country -->
            <div class="col-span-1">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Country</label>
                <input type="text" id="identCountry" value="Indonesia" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Region -->
            <div class="col-span-1">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Region/Province</label>
                <input type="text" id="identRegion" placeholder="e.g., DKI Jakarta" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Entry Date -->
            <div class="col-span-1">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Entry Date</label>
                <input type="date" id="entryDate" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Valid From (Row 2) -->
            <div class="col-span-1">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Valid From</label>
                <input type="date" id="identValidFrom" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Valid To -->
            <div class="col-span-1">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Valid To</label>
                <input type="date" id="identValidTo" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Drive Link -->
            <div class="col-span-2">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Drive Link</label>
                <input type="url" id="driveLink" placeholder="https://drive.google.com/..." class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>

            <!-- Verify Link -->
            <div class="col-span-2">
                <label class="block text-xs font-semibold text-gray-600 mb-1.5">Verify Link</label>
                <input type="url" id="verifyLink" placeholder="https://..." class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-800 bg-white">
            </div>
        </div>
    </div>
                ...
                <thead class="bg-gray-50">
                    <tr>
                        <th class="w-10 px-4 py-3 text-left">
                            <input type="radio" name="selectedIdentification" class="w-4 h-4 text-red-800 focus:ring-red-800" disabled>
                        </th>
...
<div id="confirmDeleteIdentificationModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 items-center justify-center p-4">
    <div class="bg-white rounded-xl max-w-md w-full shadow-2xl">
        <div class="p-6">
            ...
            <p class="text-sm text-gray-600 text-center mb-1">Are you sure you want to delete this identification document?</p>
...
<style>
    /* Custom dropdown style with hamburger icon */
...
<script>
    ...
    /**
     * Load all identifications for this employee
     */
    ...
    /**
     * Render identification table with data
     */
    function renderIdentificationTable(identifications) {
        const tbody = document.getElementById('identificationTableBody');
        
        tbody.innerHTML = identifications.map(ident => {
            const type = getIdentificationTypeLabel(ident.identification_type);
            const validFrom = ident.valid_from ? new Date(ident.valid_from).toLocaleDateString('en-GB') : '';
            const validTo = ident.valid_to ? new Date(ident.valid_to).toLocaleDateString('en-GB') : '';

            return `
                <tr class="hover:bg-gray-50 transition-colors cursor-pointer" onclick="selectIdentificationRow(${ident.identification_id}, event)">
                    <td class="px-4 py-3">
                        <input type="radio" name="selectedIdentification" value="${ident.identification_id}" 
                            onclick="selectIdentification(${ident.identification_id})" 
                            class="w-4 h-4 text-red-800 focus:ring-red-800">
                    </td>
                    <td class="px-4 py-3 text-sm text-gray-900">${type}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">${ident.identification_number || '-'}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">${ident.responsible_institution || '-'}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">${ident.country || '-'}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">${validFrom}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">${validTo}</td>
                    <td class="px-4 py-3">
                        <button onclick="loadIdentificationToForm(${ident.identification_id}); event.stopPropagation();" class="text-gray-400 hover:text-gray-600">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                            </svg>
                        </button>
    ...
    /**
     * Get human readable label for identification type
     */
    function getIdentificationTypeLabel(type) {
        const types = {
            ...
            'PASSPORT': 'Passport',
            'NPWP': 'Tax ID (NPWP)',
            'DRIVER_LICENSE': 'Driver License (SIM)',
    ...
    /**
     * Select identification from radio button
     */
    ...
    /**
     * Select identification row (when clicking on row)
     */
    ...
    async function loadIdentificationToForm(identificationId) {
        try {
            ...
            if (data.success && data.data) {
                ...
                document.getElementById('entryDate').value = ident.entry_date || '';
                document.getElementById('identValidFrom').value = ident.valid_from || '';
                document.getElementById('identValidTo').value = ident.valid_to || '';
                document.getElementById('driveLink').value = ident.drive_link || '';
                document.getElementById('verifyLink').value = ident.verify_link || '';
    ...
    function clearIdentificationForm() {
        ...
        document.getElementById('identRegion').value = '';
        document.getElementById('entryDate').value = '';
        document.getElementById('identValidFrom').value = '';
        document.getElementById('identValidTo').value = '';
    ...
    /**
     * Save identification (create or update)
     */
    async function saveIdentification() {
        ...
        if (!identificationType || !identificationNumber) {
            showNotification('Identification type and number are required', 'error');
            return;
        }
        ...
        const identificationData = {
            ...
            valid_from: document.getElementById('identValidFrom').value || null,
        ...
        try {
            const url = isUpdate 
                ? `/api/employees/${employeeId}/identifications/${identificationId}`
                : `/api/employees/${employeeId}/identifications`;
            ...
            if (data.success) {
                showNotification(
                    isUpdate ? 'Identification updated successfully!' : 'Identification created successfully!', 
                    'success'
                );
                loadIdentifications();
     ...
     */
    function copySelectedIdentification() {
    ...
    function deleteSelectedIdentification() {
        ...
        let identificationInfo = 'this identification document';
        
        if (identification) {
            const type = getIdentificationTypeLabel(identification.identification_type);
            const number = identification.identification_number || '';
            identificationInfo = `${type}${number ? ' - ' + number : ''}`;
        }
    ...
    async function confirmDeleteIdentification() {
        ...
        try {
            ...
            const response = await fetch(`/api/employees/${employeeId}/identifications/${deleteIdentificationId}/delete`, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
