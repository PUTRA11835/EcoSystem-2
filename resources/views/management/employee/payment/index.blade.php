@extends('dashboard')
@section('title', 'Employee Basic Payment')
@section('page-title', 'Employee Basic Payment')
@section('content')
...
<script>
    ...
    async function loadData() {
        ...
        await Promise.all(employees.map(async e => {
            ...
            try {
                const res = await fetch(`/api/employees/${id}/payment`, { credentials: 'same-origin' });
    ...
    function renderTable(rows) {
        ...
        tbody.innerHTML = rows.map((r, i) => `
            <tr class="hover:bg-gray-50 transition-colors border-b border-gray-100">
                ...
                <td class="px-4 py-3 text-gray-600 text-sm">${r.payment_type ?? '-'}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">${r.amount ?? '-'}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">${r.currency ?? '-'}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">${r.valid_from ?? '-'}</td>
