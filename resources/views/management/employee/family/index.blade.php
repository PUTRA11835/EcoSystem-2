@extends('dashboard')
@section('title', 'Employee Family')
@section('page-title', 'Employee Family')
@section('content')
<div class="bg-white rounded-xl p-6 shadow-sm">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b-2 border-gray-100">
        <div><h2 class="text-2xl font-bold text-gray-900">Employee Family</h2><p class="text-sm text-gray-500 mt-1">List of all employee family data</p></div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Employee Name</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Family Member Name</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Relationship</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Birth Date</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase"></th>
                </tr>
            </thead>
            <tbody id="tableBody"><tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">Loading...</td></tr></tbody>
        </table>
    </div>
...
<script>
    ...
    async function loadData() {
        ...
        await Promise.all(employees.map(async e => {
            ...
            try {
                const res = await fetch(`/api/employees/${id}/family`, { credentials: 'same-origin' });
    ...
    function renderTable(rows) {
        ...
        if (!rows.length) { tbody.innerHTML = `<tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">No data</td></tr>`; return; }
        tbody.innerHTML = rows.map((r, i) => `
            <tr class="hover:bg-gray-50 transition-colors border-b border-gray-100">
                ...
                <td class="px-4 py-3 text-gray-600 text-sm">${r.family_name ?? '-'}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">${r.relationship ?? '-'}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">${r.birth_date ?? '-'}</td>
