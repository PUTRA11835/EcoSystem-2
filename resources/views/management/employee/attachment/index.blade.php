@extends('dashboard')
@section('title', 'Employee Attachment')
@section('page-title', 'Employee Attachment')
@section('content')
<div class="bg-white rounded-xl p-6 shadow-sm">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b-2 border-gray-100">
        <div><h2 class="text-2xl font-bold text-gray-900">Employee Attachment</h2><p class="text-sm text-gray-500 mt-1">List of all employee attachments</p></div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">File Name</th>
...
<script>
    ...
    async function loadData() {
        ...
        await Promise.all(employees.map(async e => {
            ...
            try {
                const res = await fetch(`/api/employees/${id}/attachments`, { credentials: 'same-origin' });
    ...
    function renderTable(rows) {
        ...
        tbody.innerHTML = rows.map((r, i) => `
            <tr class="hover:bg-gray-50 transition-colors border-b border-gray-100">
                ...
                <td class="px-4 py-3 text-gray-600 text-sm">${r.file_name ?? '-'}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">${r.file_type ?? '-'}</td>
                <td class="px-4 py-3 text-gray-600 text-sm">${r.file_size ?? '-'}</td>
