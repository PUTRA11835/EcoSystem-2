...
<div class="bg-white rounded-xl p-6 shadow-sm">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b-2 border-gray-100">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Project Type</h2>
            <p class="text-sm text-gray-500 mt-1">Manage the list of project types available on the Delivery Project create/edit form.</p>
        </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 mb-4">
        <input type="text" id="typeSearch" placeholder="Search project type name..."
            oninput="renderTypes()"
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Project Type</th>
...
<!-- ── Modal: Create / Edit Project Type ──────────────────────────────────── -->
<div id="typeModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
        <div class="flex justify-between items-center p-6 border-b border-gray-100">
            <h3 id="typeModalTitle" class="text-lg font-bold text-gray-900">Add Project Type</h3>
        ...
        <form id="typeForm" onsubmit="submitType(event)" class="p-6 space-y-4 overflow-y-auto">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Project Type Name <span class="text-red-500">*</span></label>
                <input type="text" id="typeName" required maxlength="100"
                    ...
                    placeholder="e.g. Implementation, Roll Out, Body Hire">
            </div>
            <div>
...
async function loadTypes() {
    const res  = await fetch('/api/delivery-project-types');
...
function renderTypes() {
    ...
    document.getElementById('typeCount').textContent = filtered.length + ' project type(s)';

    if (!filtered.length) {
        tbody.innerHTML = `<tr><td colspan="5" class="px-4 py-8 text-center text-gray-400">${query || status ? 'No project types match your filter.' : 'No project types yet. Click "Add Project Type".'}</td></tr>`;
        return;
    }
...
function openCreateTypeModal() {
    document.getElementById('typeModalTitle').textContent = 'Add Project Type';
...
function openEditTypeModal(id) {
    ...
    if (!t) return;
    document.getElementById('typeModalTitle').textContent = 'Edit Project Type';
...
async function submitType(e) {
    ...
    const url    = id ? `/api/delivery-project-types/${id}` : '/api/delivery-project-types';
...
async function deleteType(id) {
    ...
    try {
        const res  = await fetch(`/api/delivery-project-types/${id}/delete`, { method: 'POST', headers: jsonHeaders() });
        const json = await res.json();
        if (json.success) {
            showToast(json.message || 'Project type deleted successfully.', 'success');
            loadTypes();
        } else {
            showToast(json.message || 'Failed to delete project type.', 'error');
        }
    } catch (e) {
        showToast('An error occurred. Please try again.', 'error');
    }
